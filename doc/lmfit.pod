...
B<typedef struct { double> I<ftol>B<; double> I<xtol>B<; double> I<gtol>B<;
                 ...
                 double> I<fnorm>B<; int> I<nfev>B<; int> I<info>B<; } lm_control_type;>

Default settings are provided by

B<extern const lm_control_type> I<lm_control_float>B<,> I<lm_control_double>B<;>


Status messages (indexed by I<control.info>) are available through
...
B<void lmcurve_fit( int> I<m_dat>B<, int> I<n_par>B<, double> I<*par>B<,
                  double> I<*t>B<, double> I<*y>B<, double (>I<*f>B<)(...),
                  lm_control_type> I<*control>B< );>
...
With version 3.0, the application programming interface has changed.

Generic minimization and one-dimensional curve fitting have been separated more clearly, and the interface for the latter case has been greatly simplified (see lmcurve.h and demo/curve1.c). The data type B<lm_data_type_default> and the routines B<lm_evaluate_default> and
B<lm_print_default> have become obsolete; they will be eliminated in
one of the next releases.
The initialization routine B<lm_initialize_control(...)> also has become obsolete, replacement being provided by the constant record I<lm_control_double>.
...
may be called with argument I<*printout> set to 0.


I<data>: B<lm_minimize> does not care about this pointer; it just forwards it to I<evaluate> and I<printout>;

I<control>: a record holding control parameters and monitoring variables, as described in file I<lmmin.h>.
Depending on whether I<*evaluate> computes I<fvec>
to single or double precision,
recommended default settings are in I<lm_control_float> and I<lm_control_double>.

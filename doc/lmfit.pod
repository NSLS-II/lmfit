...
B<void lmmin( int> I<m_dat>B<, int> I<n_par>B<, double> I<*par>B<, void> I<*data>B<,
            void (>I<*evaluate>B<)(...), void (>I<*printout>B<)(...),
            const lm_control_struct> I<*control>B<,
...
Generic minimization and one-dimensional curve fitting have been separated more clearly, and the interface for the latter case has been greatly simplified (see lmcurve.h and demo/curve1.c). Furthermore, control (input) and monitoring (output) parameters are now stored in two different records.

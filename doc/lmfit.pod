...
Status messages, indexed by I<status.info>, are supplied in
...
dimension of parameter vector I<par>;
...
dimension of residue vector I<fvec>;

=item I<data>

B<lm_minimize> does not care about this pointer; it just forwards it to I<evaluate> and I<printout>;

=item I<evaluate>

a routine that calculates the residue vector I<fvec> for given parameter vector I<par>; setting *I<info> to a negative value causes B<lm_minimize> to terminate;
...
=item B<int> I<control.maxcall>

maximum number of iterations;

=item B<int> I<control.printflags>
...
status of minimization; for explanation print I<lm_infmsg>B<[>I<status.info>B<]>;

=back

=item I<printout>

a routine that can be used to inform about the progress of the minimization (I<iflag>: location of call within B<lm_minimize>, I<iter>: outer loop counter, I<nfev>: number of calls to I<evaluate>);
...
By default, installation goes to B</usr/local>.
Make sure LD_LIBRARY_PATH contains B</usr/local/lib>.
In some Linux distributions, it is necessary to run B<sudo ldconfig>
...
Joachim Wuttke: lmfit - a C/C++ routine for Levenberg-Marquardt minimization with wrapper for least-squares curve fitting, based on work by B. S. Garbow, K. E. Hillstrom, J. J. MorE<eacute>, and S. Moshier. Version <..>, retrieved on <..> from http://joachimwuttke.de/lmfit/.
